...
open class SSASwiftReachability {
    typealias ReachabilityStatusChangedClosure = (ReachabilityStatus) -> Void
    ...
    fileprivate init(reachabilityRef: SCNetworkReachability, reachabilityAssociation: ReachabilityAssociation) {
        networkReachability = reachabilityRef
    ...
    func startMonitoring() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                notificationCenter.post(name: Notification.Name(rawValue: SSAReachabilityDidChangeNotification), object: self, userInfo: [SSAReachabilityNotificationStatusItem: "\(status)"])
            }
        }
    }
    ...
    func reachabilityCallback(_ flags: SCNetworkReachabilityFlags) {
        ...
        guard status != networkReachabilityStatus else { return }
